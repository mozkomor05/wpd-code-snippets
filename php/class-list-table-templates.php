 ...
 * @package Code_Snippets
 */
class Code_Snippets_List_Table_Templates extends WP_List_Table {
	...
	public function get_action_link( $action, $snippet, $escape = true ) {
		...
		if ( 'edit' === $action ) {
			return code_snippets()->get_snippet_template_edit_url( $snippet->id, $network_redirect ? 'network' : 'self' );
		}
	...
	protected function column_name( $snippet ) {
		...
		if ( $this->is_network || ! $snippet->network || current_user_can( code_snippets()->get_network_cap_name() ) ) {
			...
			$out = sprintf(
				...
				code_snippets()->get_snippet_template_edit_url( $snippet->id, $snippet->network ? 'network' : 'admin' ),
				$out
			);
		}
	...
	private function perform_action( $id, $action ) {
		...
		switch ( $action ) {
			...
			case 'clone':
				$this->clone_snippet_templates( array( $id ) );

				return 'cloned';

			case 'delete':
				delete_snippet_template( $id, $this->is_network );

				return 'deleted';

			case 'export':
				export_snippets( array( $id ), '', true );
				break;
	...
	public function process_requested_actions() {
		...
		switch ( $this->current_action() ) {
			...
			case 'export-selected':
				export_snippets( $ids, '', true );
				break;

			case 'clone-selected':
				$this->clone_snippet_templates( $ids );
				$result = 'cloned-multi';
				break;
	...
	public function prepare_items() {
		...
		/* Fetch all snippets */
		$snippets['all'] = apply_filters( 'code_snippets/list_table/get_snippets', get_snippet_templates( array() ) );
	...
	private function clone_snippet_templates( $ids ) {
		$snippets = get_snippet_templates( $ids, $this->is_network );
